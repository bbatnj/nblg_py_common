...
#        pdb.set_trace()
        ...
        cfg_copy['logFilePath']  = '' #quill log to stdout then compressed
...
'''
    ...
    #    script_template += '{sim_dir}/NblgCryptoSim {config_path} > /dev/null'
    #else:

    sim_dir = config.get('simDir')
#    gz_fn = f'{config["startDate"]}_{config["endDate"]}.stdout.txt.gz'
#    script_template += '{sim_dir}/NblgCryptoSim {config_path} 2>&1 | nice gzip -9 -c > ' + f'{config["logDir"]}/{gz_fn}'
#    print(config['logDir'])
    logFilePath = config['logFilePath']
    script_template += f'nice {sim_dir}/NblgCryptoSim {config_path} 2>&1 | nice gzip -9 -c > {config["logDir"]}/{config["startDate"]}_{config["endDate"]}.gz'
#    script_template += f'| gzip -9 -c > {logFilePath}'
#    print(config)

#    script_template += f'gzip -9 {logFilePath}'

...
def get_config(cfg_path, title, ts=None):
    ...
    assert os.path.isabs(log_dir), 'use absolute path for logDir'
    ...
    # simlibcfg copy
    assert os.path.isabs(config['libConfigPath']), 'use absolute path for libConfigPath'
    simLibCfgCopyPath = os.path.join(log_dir, os.path.basename(config['libConfigPath']))
    copyfile(config['libConfigPath'], simLibCfgCopyPath)
    copyfile(cfg_path, os.path.join(log_dir, os.path.basename(cfg_path)))
    config['libConfigPath'] = simLibCfgCopyPath
...
def prepare_sim_configs(config: dict):
    ...
    for i in range(n):
        ...
        cfg_copy['logFilePath'] = f"{cfg_copy['logDir']}/{cfg_copy['startDate']}_{cfg_copy['endDate']}.log"
...
/mnt/sda/NAS/Release/external/legacy/hdf5-1.8.18/lib:\
/mnt/sda/NAS/Release/external/boost/v1.64.0/lib:\
/mnt/sda/NAS/Release/external/snappy/v1.1.9/lib:\
/mnt/sda/NAS/Release/shennong/data_architecture/data_core/dev_global.dev/latest/lib-x64-gcc-5.4.0-Release:\
/mnt/sda/NAS/Release/external/jsoncpp/v1.9.1/lib:\
{sim_dir}:\
{libnblg_dir}

ulimit -c unlimited

{sim_dir}/sim {config_path} > /dev/null

'''
#nice gzip -9 {config_path}

    sim_dir = config.get('simDir') # assert libshennong_trading_strategy_hft_crypto_sim.so exists also
    if sim_dir:
        if not os.path.isabs(sim_dir):
            sim_dir = cwd / sim_dir
    print('sim dir', sim_dir)
    libnblg_dir = config.get('libnblgDir', sim_dir)
    assert sim_dir and libnblg_dir
    stdout_file = os.path.join(config['logDir'], config['stdout'])
    scripts_txt = script_template.format(
        sim_dir=sim_dir, 
        libnblg_dir=libnblg_dir, 
    ...
    with open(os.path.join(config['logDir'], 'debug.sh'), 'w') as f_debug:
        ...
        sim_executable_path = os.path.realpath(f'{sim_dir}/sim')
...
def compress_logs(wd):
    compress_script = '''find . \( -name "*.log" -o -name "*.txt" \) -print0 | xargs -0 -n 1 -P 0 gzip'''
